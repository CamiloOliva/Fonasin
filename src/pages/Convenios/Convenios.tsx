      ...
      <div className="container-page">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-9">
          {items.map((c) => {
            ...
            const card = (
              <article
                className={`group relative h-full min-w-0 overflow-hidden rounded-2xl border bg-white p-6 shadow-sm transition duration-200 ${
                  isEmi
                    ...
                    : isLosOlivos
                      ...
                      : isSanitas
                        ...
                        : isEmermedica
                          ...
                          : isTourism
                          ? 'border-sky-200 bg-gradient-to-br from-white via-white to-cyan-50 hover:-translate-y-1 hover:border-sky-400 hover:shadow-xl hover:shadow-sky-900/10'
              ...
              >
                {isFeatured && (
                  <span className={`absolute right-4 top-4 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider ${
                    isEmi ? 'bg-red-50 text-red-600' : isSanitas ? 'bg-sky-100 text-sky-700' : isEmermedica ? 'bg-blue-100 text-blue-700' : isUmaIps ? 'bg-emerald-100 text-emerald-700' : isPracticar ? 'bg-red-100 text-red-700' : isCoorserpark ? 'bg-emerald-100 text-emerald-700' : isTourism ? 'bg-sky-100 text-sky-700' : 'bg-[#fefce8] text-[#556b2f]'
                  }`}>
                    {isEmi ? 'Salud 24/7' : isSanitas ? 'Plan Premium' : isEmermedica ? 'Atención médica' : isPracticar ? 'Formación vial' : isCoorserpark ? 'Previsión exequial' : isTourism ? 'Agencia de viajes' : 'Protección familiar'}
                  </span>
                )}
                <img
                  src={c.logo}
                ...
                {isFeatured && (
                  <span className={`mt-5 inline-flex items-center gap-1.5 font-bold transition group-hover:gap-2.5 ${
                    isEmi ? 'text-emerald-700' : isSanitas ? 'text-sky-700' : isEmermedica ? 'text-blue-700' : isUmaIps ? 'text-emerald-700' : isPracticar ? 'text-red-700' : isManejar ? 'text-amber-700' : isCoorserpark ? 'text-emerald-700' : isTourism ? 'text-sky-700' : 'text-[#556b2f]'
                  }`}>
                    Conoce el convenio
