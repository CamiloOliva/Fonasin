      ...
      <div className="container-page">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-9">
          {items.map((c) => {
            ...
            const isCapillas = c.name === 'Capillas de la Fe';
            const isFeatured = isEmi || isLosOlivos || isSanitas || isCapillas;
            const card = (
              <article
              ...
              >
                {isFeatured && (
                  <span className={`absolute right-4 top-4 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider ${
                    isEmi ? 'bg-red-50 text-red-600' : isSanitas ? 'bg-sky-100 text-sky-700' : isCapillas ? 'bg-emerald-100 text-emerald-700' : 'bg-[#fefce8] text-[#556b2f]'
                  }`}>
                    {isEmi ? 'Salud 24/7' : isSanitas ? 'Plan Premium' : isCapillas ? 'Previsión exequial' : 'Protección familiar'}
                  </span>
                )}
                <img
                  src={c.logo}
                ...
                {isFeatured && (
                  <span className={`mt-5 inline-flex items-center gap-1.5 font-bold transition group-hover:gap-2.5 ${
                    isEmi ? 'text-emerald-700' : isSanitas ? 'text-sky-700' : isCapillas ? 'text-emerald-700' : 'text-[#556b2f]'
                  }`}>
                    Conoce el convenio
            ...
            return isFeatured ? (
              <Link
                key={c.id}
                to={isEmi ? '/convenios/emi' : isSanitas ? '/convenios/sanitas' : isCapillas ? '/convenios/capillas-de-la-fe' : '/convenios/los-olivos'}
