...
- La creacion del borrador de actualizacion del asociado se ejecuta en transaccion, bloquea la fila del asociado mientras busca o crea el borrador activo y se respalda con un indice unico parcial para impedir mas de un borrador `draft` por asociado.
...
- Las cargas XLSX de creditos y aportes validan extension, MIME, tamano, columnas obligatorias, tipos, valores, duplicados dentro del archivo y repeticion por hash de archivo. El archivo queda en storage privado bajo una clave generada por servidor, no en `public/`.
